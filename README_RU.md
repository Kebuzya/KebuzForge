...
- **Палитры** - Median Cut квантование, ретро-палитры, перекраска отдельных цветов без пересчёта, импорт/экспорт (JASC PAL, Photoshop ACT, GIMP GPL)
- **Дизеринг** - Floyd–Steinberg, Bayer 4×4 / 8×8 с настраиваемой интенсивностью, накладывается поверх эффектов
- **3D-эффекты** - сфера, цилиндр, скруглённый прямоугольник с освещением (ambient/diffuse, направление света)
- **Пиксельный редактор** - карандаш, линия, прямоугольник, окружность (Shift - ровная фигура, Ctrl - от центра), заливка, пипетка, ластик, второй цвет на ПКМ, зум до 16×
...
- **Авто-каскад конвейера** - повторная пикселизация автоматически пересчитывает палитру, дизеринг и эффекты (отключается)
- **Удаление фона**, контуры, отсоединяемые панели, тёмная/светлая/гибридная темы, пресеты, двойной клик по подписи ползунка - ввод точного значения
